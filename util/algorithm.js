...
const fcfs = (W_equal, W_plus, alpha, beta, _a, _b) => {
    ...
    while (a[a_index] !== Infinity || b[b_index] !== Infinity) {
        ...
        let possible_entering_time =
            prev_entering_time +
            (prev_lane === selected_lane || prev_lane === Lane.Out
                ? W_equal
                : W_plus);
        // console.log("selected lane is:" + selected_lane.toString())
        // console.log("possible entering time is:" + possible_entering_time.toString())
        prev_entering_time = Math.max(
            selected_lane === Lane.A ? a_now : b_now,
            possible_entering_time
        );
...
const fcfs_multiple = (W_equal, W_plus, _carLines, laneNum) => {
    ...
    while (checkCompleted()) {
        ...
        for (let i = 0; i < laneNum; i++) {
            if (
                carLines[i][index[i]] <
                carLines[selected_lane][index[selected_lane]]
            ) {
                selected_lane = i;
            }
        }
...
const dp2 = (W_equal1, W_plus1, W_equal2, W_plus2, T_f, _a, _b, _c) => {
    const Lane = {
        Out: -1,
        A: 0,
        B: 1,
    ...
    for (let i = 0; i <= alpha; i++) {
        for (let j = 0; j <= beta; j++) {
            if (i + j === 0) {
                time1[i][j][Lane.A] = time1[i][j][Lane.B] = 0;
                backtrack1[i][j][Lane.A] = backtrack1[i][j][Lane.B] = Lane.Out;
                continue;
            }
            if (i > 0) {
                ...
                if (i === 1 && !j) {
                    time1[i][j][Lane.A] = a[i];
                    backtrack1[i][j][Lane.A] = Lane.Out;
                }
            }
            if (j > 0) {
                ...
                if (j === 1 && !i) {
                    time1[i][j][Lane.B] = b[j];
                    backtrack1[i][j][Lane.B] = Lane.Out;
                }
            }
        }
    }
            ...
            for (let k = 0; k <= gamma; k++) {
                ...
                if (j > 0) {
                    ...
                    const BLanes = Lane.toList().map((lane) => {
                        ...
                        return Math.max(
                            Math.max(b[j], time1[i][j - 1][mp1Lane] + W_1) + T_f,
    ...
    for (let iter = new Iter(carCounts); !iter.end(); iter.next()) {
        ...
        indices.forEach((ind, thisLane) => {
            ...
            if (indices.reduce((prev, curr) => prev + curr) == 1) {
                time.get(indices)[thisLane] = padLines[thisLane][ind]
